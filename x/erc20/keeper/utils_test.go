...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	// Mint coins on the osmosis side which we'll use to unlock our egax
	...
	// Mint coins on the cosmos side which we'll use to unlock our egax
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/egax") as input parameter.
