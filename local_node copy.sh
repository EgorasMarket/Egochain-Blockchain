...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	VAL_KEY="mykey"
	VAL_MNEMONIC="blouse all crop alert emerge nut twist card slam problem sphere salad mirror mango cute evolve already lady confirm rate wrap solid valley saddle"
	...
	# Change parameter token denominations to egax
	jq '.app_state["staking"]["params"]["bond_denom"]="egax"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["crisis"]["constant_fee"]["denom"]="egax"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="egax"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	# When upgrade to cosmos-sdk v0.47, use gov.params to edit the deposit params
	jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="egax"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["evm"]["params"]["evm_denom"]="egax"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["inflation"]["params"]["mint_denom"]="egax"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	# jq -r --arg amount_to_claim "$amount_to_claim" '.app_state["bank"]["balances"] += [{"address":"egochain15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz","coins":[{"denom":"egax", "amount":$amount_to_claim}]}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	evmosd add-genesis-account "$(evmosd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 100000000000000000000000000egax --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmosd add-genesis-account "$(evmosd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000egax --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmosd add-genesis-account "$(evmosd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000egax --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmosd add-genesis-account "$(evmosd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000egax --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmosd add-genesis-account "$(evmosd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000egax --keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	evmosd gentx "$VAL_KEY" 1000000000000000000000egax --gas-prices ${BASEFEE}egax --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
...
evmosd start \
	--metrics "$TRACE" \
	--json-rpc.address="0.0.0.0:4590" \
	--json-rpc.ws-address="0.0.0.0:4591" \
	--log_level $LOGLEVEL \
	--minimum-gas-prices=0.0001egax \
