...
def ibc(request, tmp_path_factory):
    """
    prepare IBC network with an dhives chain
...
def test_ibc_transfer_with_hermes(ibc):
    ...
    dst_addr = ibc.chains["dhives"].cosmos_cli().address("signer2")
    old_dst_balance = get_balance(ibc.chains["dhives"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["dhives"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("dhives"))
...
def test_evmos_ibc_transfer(ibc):
    """
    test sending dhives from dhives to crypto-org-chain using cli.
    """
    assert_ready(ibc)
...
def test_evmos_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending dhives from evmos to crypto-org-chain using cli
