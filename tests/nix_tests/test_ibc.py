...
def ibc(request, tmp_path_factory):
    """
    prepare IBC network with an egax chain
...
def test_ibc_transfer_with_hermes(ibc):
    ...
    dst_addr = ibc.chains["egax"].cosmos_cli().address("signer2")
    old_dst_balance = get_balance(ibc.chains["egax"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["egax"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("egax"))
...
def test_evmos_ibc_transfer(ibc):
    """
    test sending egax from egax to crypto-org-chain using cli.
    """
    assert_ready(ibc)
...
def test_evmos_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending egax from evmos to crypto-org-chain using cli
