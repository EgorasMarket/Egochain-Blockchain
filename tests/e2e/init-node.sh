...
CHAINID="${CHAIN_ID:-dhives_5438-1}"
...
# Change parameter token denominations to dhives
jq '.app_state.staking.params.bond_denom="dhives"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.crisis.constant_fee.denom="dhives"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.gov.deposit_params.min_deposit[0].denom="dhives"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.evm.params.evm_denom="dhives"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.inflation.params.mint_denom="dhives"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
if jq '.app_state.gov.params != null' "$GENESIS" | grep -q "true"; then
  jq '.app_state.gov.params.min_deposit[0].denom="dhives"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
jq -r --arg amount_to_claim "$amount_to_claim" '.app_state.bank.balances += [{"address":"evmos15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz","coins":[{"denom":"dhives", "amount":$amount_to_claim}]}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
evmosd add-genesis-account "$(evmosd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING")" 100000000000000000000000000dhives --keyring-backend "$KEYRING"
evmosd add-genesis-account "$(evmosd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000dhives --keyring-backend "$KEYRING"
evmosd add-genesis-account "$(evmosd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000dhives --keyring-backend "$KEYRING"
evmosd add-genesis-account "$(evmosd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000dhives --keyring-backend "$KEYRING"
evmosd add-genesis-account "$(evmosd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000dhives --keyring-backend "$KEYRING"
...
evmosd gentx "$VAL_KEY" 1000000000000000000000dhives --gas-prices ${BASEFEE}dhives --keyring-backend "$KEYRING" --chain-id "$CHAINID"
...
evmosd start "$TRACE" \
  --log_level $LOGLEVEL \
  --minimum-gas-prices=0.0001dhives \
